...
describe('GET /getTagsWithQuestionNumber', () => {
  ...
  it('should throw error if error occurs getting tags', async () => {
    // Mocking Tag.find() and Question.find()
    Tag.find = jest.fn().mockResolvedValueOnce(mockTags);

    Question.find = jest.fn().mockImplementation(() => ({populate: jest.fn().mockRejectedValue(new Error("Fail"))}));
    ...
    // Asserting the response body
    expect(response.body).toEqual({error: "Unable to get tags and question numbers"});
  });
